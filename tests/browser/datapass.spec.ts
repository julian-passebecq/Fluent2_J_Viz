...
test('real Fluent surfaces, lazy editor, keyboard controls, reduced motion and SVG action', async ({ page }) => {
  const errors: string[] = [];
  page.on('pageerror', error => errors.push(error.message));
  await page.goto('/');
  await expect(page.locator('.dp-app-shell .dp-workbench [data-figure-renderer="vizforge.d3"]')).toBeVisible();
  await expect(page.locator('.dp-figure-player')).toHaveCount(1);
  await expect(page.locator('.vf-controls')).toHaveCount(0);
  expect(await page.evaluate(() => performance.getEntriesByType('resource').some(r => /MonacoSurfaces|editor.worker/.test(r.name)))).toBe(false);
  ...
  await expect(page.getByRole('button', { name: 'Play unavailable while reduced motion is enabled', exact: true })).toBeDisabled();
  ...
  expect(await page.evaluate(() => performance.getEntriesByType('resource').some(r => /MonacoSurfaces/.test(r.name)))).toBe(true);
