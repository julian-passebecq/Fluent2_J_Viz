...
const reactDom = sources.filter(source => /\/react-dom\/cjs\/react-dom-client\.production\.js$/.test(source));
...
assert(!sources.some(source => /src\/core\/player\.ts$/.test(source)), 'Hosted Studio must not ship a VizForge StoryPlayer clock');
assert(sources.some(source => /packages\/figure\/src\/player\.tsx$/.test(source)), 'Real Datapass FigurePlayer must ship');
for (const file of readdirSync('dist/assets').filter(file => file.endsWith('.map'))) {
  ...
  assert(!map.sources.some(source => /^(?:[A-Z]:[\\/]|\/Users\/|\/home\/)/i.test(source)), `Absolute source path: ${file}`);
  const content = (map.sourcesContent ?? []).join('\n').replaceAll('\\', '/').toLowerCase();
  assert(!content.includes(process.cwd().replaceAll('\\', '/').toLowerCase()), `Workspace path leak: ${file}`);
}
const report = { framework: JSON.parse(readFileSync('datapass.json')).commit, privacy, reactCore, reactDom,
  studioContainsVizForgeClock: false, studioContainsDatapassPlayer: true };
mkdirSync('docs/qa', { recursive: true });
writeFileSync('docs/qa/bundle-audit.json', JSON.stringify(report, null, 2) + '\n');
console.log('Bundle audit passed: private URL/source maps, one React runtime, Datapass-only playback ownership.');
