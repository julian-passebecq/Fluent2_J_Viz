...
for (const entry of [...catalog, ...storyPacks]) parseStory(entry.story);
parseStory(editorialStory);
const provenance = JSON.parse(readFileSync('docs/DATA_PROVENANCE.json', 'utf8'));
deepStrictEqual(provenance.fixtures.length, catalog.length + storyPacks.length + 1);
for (const fixture of provenance.fixtures) {
  deepStrictEqual(fixture.synthetic, true);
  deepStrictEqual(
    fixture.sha256,
...
for (const entry of [...catalog, ...storyPacks, { id: 'editorial', story: editorialStory }]) {
  deepStrictEqual(
...
console.log(
  `Validated ${catalog.length} families and ${catalog.length + storyPacks.length + 1} canonical visual/story examples.`,
);
